...
## Créneaux à double validation — installés le 19 septembre, recette en cours

- Recette utilisateur : la proposition apparaît dans les deux comptes et Antoine confirme la création dans l’agenda après double accord. Le contrôle serveur constate aussi une proposition avec deux choix différents, toujours en attente et sans événement Google confirmé. Le manque de mention explicite a été corrigé : « Sans accord commun » pour des choix différents, et indication de l’accord manquant sinon.
- Correctif web `533dea3578414c7ea66fe22d3bad10c99b7d73fa` installé le 19 septembre : seul le portail recréé ; API conservée à `05fd76b`, Alexa à `57aa69d`, leurs conteneurs inchangés. Six services sains, Nginx valide, libellé et style corrigés servis en HTTPS 200. Huit tests web, affichage des quatre états et largeurs 320/390 px réussis ; aucune erreur console. CI PostgreSQL et GitGuardian réussis sur 533dea3. Prochaine vérification utilisateur : actualiser Maison et retrouver « Sans accord commun » sur la proposition aux choix différents. PR 8 reste en brouillon, T06 En cours ; recette Google sur le téléphone de Belinda toujours reportée.

- Installation autorisée par Antoine (« oui »), sauvegarde locale complète puis mise en service à `05fd76b7505dc698537730f2f31f1980c5089472`. Migration 005 appliquée et API/web actualisés ; Alexa conserve 57aa69d. Six services Maison sains. Accueil/module propositions HTTPS 200 ; propositions, agenda et courses anonymes 401 ; ancienne API événements et administration 404. Lecture Google depuis l’API installée réussie, deux membres présents et aucune proposition au contrôle. Aucun rendez-vous Google créé pendant cette installation.
- Sauvegarde vérifiée par empreintes, puis les deux bases restaurées dans un conteneur isolé avec nombres d’enregistrements conformes. Le contrôle initial a révélé la détection trop précoce du serveur PostgreSQL temporaire ; attente corrigée, contrôle rejoué avec succès, correctif 05fd76b. Cela valide les bases, pas la restauration du service complet. Destination hors serveur toujours différée. Sauvegarde avec bref arrêt/redémarrage des services applicatifs Maison ; aucun service tiers, DNS, partage Google ou coût modifié.
- À l’installation initiale, CI PostgreSQL et GitGuardian réussis sur 05fd76b ; premier test de partage demandé. Les retours de recette ultérieurs figurent ci-dessus. Continuer un test à la fois, sans refaire les essais antérieurs.

### Historique — préparation avant installation
...
- Compte propriétaire choisi : Antoine. Nouvel agenda privé « Maison - Famille » créé, fuseau Europe/Paris ; agenda Famille préexistant préservé. Les seuls événements des essais techniques ont été supprimés après vérification.
...
- [PR 8 — Google Agenda](https://github.com/AntoBel4/Commandement-Center/pull/8) ouverte en brouillon, tête `6534c0c6368eca9d8b929dbfa3a5d3bd4899dd4f`. Lecture directe de l’agenda dédié dans Maison, création datée ou journée entière, récurrences développées, changements et annulations Google relus, détails privés masqués. Reprise d’un envoi sans doublon après perte de réponse ; refus des horaires Paris inexistants ou ambigus. Modification/annulation/répétition se gèrent dans Google via le lien du portail.
- Validation : 44 tests locaux réussis, deux intégrations optionnelles ignorées dans cette commande ; CI avec PostgreSQL et contrôle des scripts réussie sur la tête. Formulaire vérifié à 320/390 px et page à 1280 px. Tests réels Google depuis Nexus : création et reprise unique, modification relue, deux occurrences récurrentes, annulation d’une occurrence et nettoyage final réussis. Aucun événement de test technique conservé. Le droit limité refuse de rendre un événement privé ; masquage couvert par tests locaux, recette privée réelle non effectuée.
- Sauvegarde locale complète avant installation, empreintes vérifiées ; restauration de ce nouveau jeu non effectuée. Une erreur de syntaxe de l’extension du script de sauvegarde a été corrigée avant installation, les services avaient redémarré correctement. API/web Maison installés à `6534c0c` ; six services sains, Alexa conserve `57aa69d`. La sauvegarde a brièvement arrêté les services applicatifs Maison ; aucun service tiers redémarré, aucune modification DNS/Cloudflare ou facturation.
- Lecture Google depuis l’API installée réussie. Après démarrage et contrôles de santé : HTTPS accueil/module agenda 200, courses et agenda anonymes 401, ancienne API événements et administration 404. Clé montée en lecture seule, hors dépôt ; aucun secret publié.
- Quatre essais sur le téléphone d’Antoine sont acceptés : ajout depuis Maison visible une seule fois dans Google ; modification Google à 11 h–12 h relue dans Maison sans doublon ; répétition quotidienne de deux occurrences correctement affichée ; suppression de la seule occurrence de demain répercutée, celle d’après-demain conservée à 11 h–12 h. Vérification sur le téléphone de Belinda reportée explicitement par Antoine, qui n’a pas accès à son agenda ; aucun succès déduit pour ce téléphone. L’occurrence de test restante est conservée pour cette vérification ultérieure. Créneaux à double validation, rappels Telegram et rendez-vous vocaux restent à réaliser. T06 En cours. [Guide et retour arrière](https://github.com/AntoBel4/Commandement-Center/blob/feat/google-calendar/docs/GOOGLE-AGENDA.md).
- CI PostgreSQL et GitGuardian réussis sur la tête 6534c0c ; aucun commentaire ou fil de revue GitHub en attente. PR 8 en brouillon pendant la recette partagée.
