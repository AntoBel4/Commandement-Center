...
- Recette en cours : proposition visible dans les deux comptes selon Antoine ; un premier accord du second compte effectivement enregistré (lecture serveur : en attente, version 2, deux créneaux, un accord, aucun accord du créateur, aucun événement Google confirmé). Antoine signale l’absence de changement visible sur son compte. L’absence de création est normale sans second accord ; la visibilité de l’accord reçu reste à vérifier après « Actualiser les propositions ». Ne pas déduire une double validation ni un défaut de persistance.
...
- CI PostgreSQL et GitGuardian réussis sur 05fd76b. PR 8 reste en brouillon ; T06 En cours. Premier test utilisateur demandé : partager deux créneaux futurs depuis le compte d’Antoine sans valider ; attendre zéro accord et aucun événement Google. Résultat encore attendu. Continuer un test à la fois, sans refaire les essais antérieurs. Vérification du téléphone de Belinda toujours reportée.
