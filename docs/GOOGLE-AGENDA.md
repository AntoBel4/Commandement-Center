...
- Le destinataire peut répondre « Aucun des deux créneaux » (ou « Aucun de ces créneaux » selon le nombre). Cette réponse remplace son éventuel accord, persiste et apparaît dans les deux comptes avec « Sans accord commun ». Il peut retirer sa réponse ou choisir ensuite un horaire.
- Après un refus global ou deux choix différents, « Proposer d’autres créneaux » ouvre un formulaire avec titre, lieu et précisions conservés. Le partage remplace les horaires de cette même proposition et efface les deux réponses, en une seule opération. Le membre qui les partage devient le proposant ; chacun doit donner un nouvel accord. Fermer le formulaire ne change rien. Une version périmée est refusée ; après perte de réponse, la même clé reprend le remplacement sans effacer de nouveaux accords. Aucun remplacement pendant ou après la création Google.
- Avant les deux accords, chacun peut retirer sa réponse ou annuler la proposition. Pour changer les détails hors désaccord, annuler et proposer de nouveau : les anciens accords ne sont pas réutilisés.
...
Recette guidée après installation : partager deux créneaux ; vérifier leur présence depuis l’autre compte et l’absence dans Google ; donner un seul accord ; choisir des horaires différents ; réunir les deux accords sur le même horaire ; vérifier un unique événement ; essayer ensuite « Aucun des deux créneaux », sa visibilité dans les deux comptes, puis « Proposer d’autres créneaux » avec deux nouveaux accords nécessaires ; retrait et annulation sur une autre proposition.
...
L’option de refus et de remplacement utilise la table existante et exige API/web de la même nouvelle version (aligner GOOGLE_RELEASE et GOOGLE_WEB_RELEASE). Aucune migration supplémentaire. Un ancien portail reçoit un conflit si ses accords portent sur des horaires remplacés ; recharger la page.
